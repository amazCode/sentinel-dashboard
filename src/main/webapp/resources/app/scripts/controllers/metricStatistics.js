...
app.controller('MetricStatisticsCtl', ['$scope', '$stateParams', 'MetricService', '$interval',
  function ($scope, $stateParams, MetricService, $interval) {
	var time=(new Date).getTime()-3*24*60*60*1000;
	$scope.searchStartTime = new Date(time);
	$scope.searchEndTime = new Date();
...
//    	var date = $scope.searchStartTime;
...
//		$scope.searchEndTime = null;
    ...
    function queryIdentityDatas() {
      var params = {
        ...
        searchStartTime:formatDate($scope.searchStartTime),
        searchEndTime:formatDate($scope.searchEndTime)
      };
      MetricService.queryResourceMetricByTime(params).success(function (data) {
        $scope.metrics = [];
        $scope.emptyObjs = [];
        if (data.code === 0 && data.data) {
          ...
          $.each(identityNames, function (idx, identityName) {
            ...
            metrics.shortData = lastOfArray(identityDatas, identityDatas.length);
